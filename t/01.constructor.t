...
subtest 'Set illegal bpp' => sub {
    ...
    throws_ok { $psnr->bpp('foo') }
    qr/'foo' is not an integer\. bpp must be an integer\./;
};

subtest 'Set illegal max_power' => sub {
    dies_ok { $psnr->_set_max_power('bar') }
    'Die cause of max_power is not integer';
    throws_ok { $psnr->_set_max_power('bar') }
    qr/'bar' is not an integer\. max_power must be an integer\./;
};
...
subtest 'Set illegal x' => sub {
    ...
    throws_ok { $psnr->x('foo') }
    qr/x must be array reference or hash reference\./;
};

subtest 'Set illegal y' => sub {
    dies_ok { $psnr->y('bar') } 'Die cause of y is not hash or array';
    throws_ok { $psnr->y('foo') }
    qr/y must be array reference or hash reference\./;
};
